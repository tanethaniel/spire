...
export function ProfileSheet({ user, stats, interpretationEnabled, onToggle, mbti, onMbtiChange, goals, onClose }: ProfileSheetProps) {
  ...
  const [isGoogleLinked, setIsGoogleLinked] = useState(true); // default true to avoid flash
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      const hasGoogle = provider === 'google' || identities.some((id: { provider: string }) => id.provider === 'google');
